...
\end{itemize}
Praca nie obejmuje implementacji generowania pełnej logiki biznesowej ciał metod, a jedynie ich sygnatur oraz struktury architektonicznej wynikającej z modelu UML.

\section{Krótki opis struktury pracy}

Praca została podzielona na dwa główne etapy: analityczny i projektowy.

Etap analityczny (rozdziały 2-3) obejmuje przegląd literatury i istniejących rozwiązań CASE, identyfikację luk rynkowych oraz sformułowanie wymagań funkcjonalnych i niefunkcjonalnych systemu.

Etap projektowy (rozdziały 4-6) opisuje architekturę trójwarstwową aplikacji, implementację kluczowych modułów (parser XMI, generator kodu Python, frontend JS), testy jednostkowe (pytest) oraz instrukcję obsługi.

Zakończenie (rozdział 7) zawiera podsumowanie, ocenę realizacji celów oraz perspektywy rozwoju, w tym m.in. rozszerzenie obsługi o inne diagramy UML i języki programowania.
...
Na podstawie analizy rynku oraz identyfikacji potrzeb użytkowników opracowano zestaw wymagań, które precyzyjnie określają zakres implementacji narzędzia i stanowią podstawę do weryfikacji zgodności projektu z założeniami pracy.

Podział wymagań na funkcjonalne (WF), niefunkcjonalne (WNF) oraz historyjki użytkownika (HU) umożliwia systematyczną analizę potrzeb programisty Python, wspiera modularne projektowanie architektury systemu oraz zapewnia obiektywne kryteria oceny stopnia realizacji założonych celów.
...
\item\label{WF-klasa1} Aplikacja umożliwia użytkownikowi dodawanie klas do diagramu.
...
\item\label{WF-klasy1} System generuje definicje klas Python zgodnie ze składnią.
...
\item\label{WF-klasy5} System używa \texttt{type hints} \cite{pep484} dla parametrów i zwracanych wartości.
...
\item\label{WNF-skalowalnosc1} System powinien obsługiwać diagramy zawierające nawet 1000 klas.
...
\item\label{WNF-standardy1} Pliki wykorzystywane przez system powinny być zgodne ze standardem XMI 2.1.
\item\label{WNF-standardy2} Wygenerowany kod powinien być zgodny z Python 3.11+.
\item\label{WNF-standardy3} System powinien używać \texttt{type hints} zgodnie z PEP 484.

\item\label{WNF-integracja1} System powinien obsługiwać import modeli z innych narzędzi UML eksportujących pliki w formacie XMI 2.1.
\item\label{WNF-integracja2} Wygenerowany kod musi tworzyć strukturę pakietów Python zgodną ze standardami setuptools i pip
...
W trakcie prac nad projektem pojawiły się liczne pomysły na rozwój i rozszerzenie funkcjonalności systemu. Część z nich wynikała z analizy ograniczeń obecnej implementacji, a część z obserwacji potencjalnych kierunków wykorzystania narzędzia w szerszym kontekście inżynierii oprogramowania. Poniżej przedstawiono przykładowe propozycje możliwych usprawnień i rozszerzeń.
...
W toku realizacji projektu wyłoniły się również koncepcje potencjalnych kierunków badań. Ich realizacja mogłaby dostarczyć danych na temat skuteczności i praktycznego zastosowania podejścia MDD w środowisku Pythona, a także pozwolić na porównanie go z tradycyjnymi metodami wytwarzania oprogramowania.
