export default class Exporter {
  buildXMI(state, modelName) {
    ...
    state.packages.forEach(pkg => {
      const pe = doc.createElement('packagedElement');
    ...
    state.classes.forEach(cls => {
      const el = doc.createElement('packagedElement');
      ...
      cls.attributes.forEach((a, i) => {
        ...
        const attr = doc.createElement('ownedAttribute');
      ...
      cls.operations.forEach((o, i) => {
        ...
        const op = doc.createElement('ownedOperation');
      ...
      state.relations.filter(r => r.type === 'generalization' && r.source === cls.id).forEach((r, i) => {
        const gen = doc.createElement('generalization');
    ...
    state.relations.filter(r => ['association','aggregation','composition'].includes(r.type)).forEach((r, i) => {
      const assoc = doc.createElement('packagedElement');
      ...
      const endA = doc.createElement('ownedEnd');
      ...
      const endB = doc.createElement('ownedEnd');
    ...
    state.relations.filter(r => r.type === 'dependency').forEach((r, i) => {
      const dep = doc.createElement('packagedElement');
      dep.setAttribute('xmi:type', 'uml:Dependency');
      dep.setAttribute('xmi:id', r.id);
      dep.setAttribute('name', `Dependency${i + 1}`);
      const client = doc.createElement('client'); client.setAttribute('xmi:idref', r.source);
      const supplier = doc.createElement('supplier'); supplier.setAttribute('xmi:idref', r.target);
    ...
    state.relations.filter(r => r.type === 'realization').forEach((r, i) => {
      const rea = doc.createElement('packagedElement');
