...
export default class LinkService {
  ...
  toggle(button) {
    ...
    if (button) button.textContent = this.state.linkMode ? 'End Link' : 'Start Link';
  }
  beginLinkOnClass(c, linkType, onAddRelation) {
    ...
    if (!this.state.tempEdge) {
      ...
      this.state.tempEdge = l; this.state.tempEdge.dataset.source = c.id;
    } else {
      const src = this.state.tempEdge.dataset.source;
      onAddRelation(linkType, src, c.id);
      this.state.tempEdge.remove(); this.state.tempEdge = null;
      this.state.linkMode = false;
    }
  }
  updatePreview(svg, viewport, clientX, clientY) {
    if (!this.state.tempEdge) return;
    const src = (this.state.tempEdge.dataset?.source) ? this.state.classById?.(this.state.tempEdge.dataset.source) : null;
    if (!src) return;
  ...
  cancel(button) {
    if (this.state.tempEdge) { this.state.tempEdge.remove(); this.state.tempEdge = null; }
    this.state.linkMode = false;
