export default class EditorsUI {
  constructor(state, refs, onChange) {
    ...
    refs.btnDelete.addEventListener('click', () => {
      ...
      if (s?.type === 'class') { this.state.removeClass(s.id); this.state.clearSelection(); this.onChange(); }
    });
    refs.btnPkgUpdate.addEventListener('click', () => this.applyPackageEditor());
    refs.btnPkgDelete.addEventListener('click', () => {
      const s = this.state.selected;
      if (s?.type === 'package') { this.state.removePackage(s.id); this.state.clearSelection(); this.refreshPackageSelect(); this.onChange(); }
    });
  }
  refreshPackageSelect() {
    ...
    this.state.packages.forEach(p => { const o = document.createElement('option'); o.value = p.id; o.textContent = p.name; inClsPkg.appendChild(o); });
    if (val) inClsPkg.value = val;
  }
  applyClassEditor() {
    const sel = this.state.selected; if (!sel || sel.type !== 'class') return;
  ...
  applyPackageEditor() {
    const sel = this.state.selected; if (!sel || sel.type !== 'package') return;
  ...
  updateEditors() {
    const { noSel, classEditor, packageEditor, inClsName, inAttrs, inOps, inClsPkg, inPkgName } = this.refs;
    const sel = this.state.selected;
    if (!sel) {
      noSel.classList.remove('hidden'); classEditor.classList.add('hidden'); packageEditor.classList.add('hidden'); return;
    }
    ...
    if (sel.type === 'class') {
      ...
      this.refreshPackageSelect(); inClsPkg.value = c.packageId || '';
    } else {
      packageEditor.classList.remove('hidden'); classEditor.classList.add('hidden');
      const p = this.state.packageById(sel.id); inPkgName.value = p.name;
    }
  }
}