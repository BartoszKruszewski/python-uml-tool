export default class DiagramState {
  ...
  uid(prefix) { return prefix + (this.nextId++); }
  classById(id) { return this.classes.find(c => c.id === id); }
  packageById(id) { return this.packages.find(p => p.id === id); }
  setSelected(type, id) { this.selected = { type, id }; }
  clearSelection() { this.selected = null; }
  addClass(x, y) {
    const id = this.uid('C');
    const c = { id, name: 'Class' + this.nextId, x, y, w: 200, h: 110, attributes: [], operations: [], packageId: null };
    this.classes.push(c);
    return c;
  }
  addPackage(x, y) {
  ...
  removePackage(id) {
    ...
    this.classes.forEach(c => { if (c.packageId === id) c.packageId = null; });
  }
